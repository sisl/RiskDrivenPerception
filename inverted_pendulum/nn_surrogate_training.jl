...
# Train
Flux.@epochs 1000 Flux.train!(loss, θ, data, opt, cb = throttlecb)

model = model |> cpu
BSON.@save "inverted_pendulum/risk_networks/risk_net_k1_0_unnorm.bson" model
...
heatmap(ϵ1s, ϵ2s, (x, y) -> CVaR([-0.2, 0], [x, y], 0.0), clims = (0, π))
