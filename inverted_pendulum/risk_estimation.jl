...
p1 = histogram(samples, title="Pendulum Costs", label="MC", xlabel="|θ|", bins=range(0, π, step=0.1), alpha=0.3, normalize=true)
...
p2 = histogram!(cost_points, weights=Uw[si], normalize=true, bins=range(0, π, step=0.1), alpha=0.4, label="Dynamic Programming")
# plot(p1, p2)
...
heatmap(-1:0.1:1, -3:0.1:3, (x, y) -> normalized_CVaR([0.2, 0], [x, y], 0), title="α = 0", clims=(0, π))
